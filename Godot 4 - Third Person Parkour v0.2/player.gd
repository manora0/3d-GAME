...
#Player model
@onready var player_main_model : Node3D = $model2
@onready var mesh : Node3D = $model2/SK_UEFN_Mannequin

#AnimationTree
@onready var animation_player = $model2/AnimationPlayer
...
var slide_had_input: bool = false
...
func _input(event):
	...
	if Input.is_action_just_pressed("slide") and is_on_floor() and canmove and !is_sliding:
		start_slide()
...
func wallrun_check(delta):
	...
	if not is_on_floor() and (on_wall_rh or on_wall_lh) and horizontal_speed > move_speed * 0.5:
		if not is_wallrunning:
			...
			# small vertical boost on wallrun entry
			vertical_velocity.y = max(vertical_velocity.y, jump_force * 0.4)
...
func animation_parameter():
	...
	animation_tree.set("parameters/conditions/slide_start", is_sliding and not slide_out)
	animation_tree.set("parameters/conditions/slide_out", slide_out)
	animation_tree.set("parameters/conditions/slide_end", not is_sliding and not slide_out)
...
func get_input(delta):
	...
	if enable_root_motion:
		...
		if not is_wallrunning and not is_sliding:
	...
	if strafe:
		...
		run_speed = 6.0
		if direction != Vector3.ZERO:
			if !(climb or vault or mantle) and not is_wallrunning and not is_sliding:
...
func start_slide():
	...
	# capture current velocity and boost it
	slide_velocity = player_velocity.normalized() * move_speed * SLIDE_SPEED_MULTIPLIER
	animation_tree.set("parameters/SLIDE/OneShot/request", AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE)
	animation_tree.set("parameters/SLIDE/TimeSeek/seek_request", .6)
	animation_state.travel("SLIDE")

func end_slide():
	slide_had_input = inputdir != Vector3.ZERO
	canmove = false
	if slide_had_input:
		animation_tree.set("parameters/SLIDE OUT/Transition/transition_request", "out_run")
		print("pog")
	else:
		animation_tree.set("parameters/SLIDE OUT/Transition/transition_request", "out_idle")
		print("gers")
...
func slide_check(delta):
	if not is_sliding:
		return
	
	slide_timer -= delta
	
	# apply friction to bleed off speed
	slide_velocity = slide_velocity.lerp(Vector3.ZERO, SLIDE_FRICTION * delta)
	velocity.x = slide_velocity.x
	velocity.z = slide_velocity.z
	
	# exit conditions
	if slide_timer <= 0 or slide_velocity.length() < 3:
		end_slide()
		return
